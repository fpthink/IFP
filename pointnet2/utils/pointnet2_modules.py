...
class PointNet2SAModuleMSG(_PointNet2SAModuleBase):
    ...
    bn : bool
        Use batchnorm
    """
    def __init__(self, *, radii: List[float], nsamples: List[int], mlps: List[List[int]], bn: bool = True, use_xyz: bool = True,use_edge:bool=True,img_channels=0):
        super().__init__()
...
if __name__ == "__main__":
    ...
    test_module.cuda()
    print(test_module)
    num_parameters = sum(torch.numel(parameter) for parameter in test_module.parameters())
    print(num_parameters)
    # print(test_module(xyz, None,3))
    ...
    for _ in range(1):
        _, new_features = test_module(xyz, xyz_feats, 3)
        params = list(test_module.named_parameters())

        print(params[0][0], params[0][1].grad)
